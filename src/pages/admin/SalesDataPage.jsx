...
const APPS_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbx7oyzDV2zcpYgqppB1n11iyA0QF_hGYeHwvg4g1iN3ecFQZP6QGRZz1VEAuQf-VXs5/exec"
...
function AccountDataPage() {
  ...
  // New state for main view date filters
  ...
  const sortDateWise = (a, b) => {
    // Sort by Planned Date (Column M - index 12)
  ...
  // Update filteredAccountData calculation to include date filtering
  const filteredAccountData = accountData
    .filter((account) => {
      ...
      // Date range filter (Column M - index 12 for Planned Date)
      let matchesDateRange = true
      if (mainStartDate || mainEndDate) {
        const itemDate = parseDateFromDDMMYYYY(account["col12"])

        if (!itemDate) return false

        if (mainStartDate) {
          const startDateObj = new Date(mainStartDate)
          startDateObj.setHours(0, 0, 0, 0)
          if (itemDate < startDateObj) matchesDateRange = false
        }

        if (mainEndDate) {
          const endDateObj = new Date(mainEndDate)
  ...
  const filteredHistoryData = historyData
    .filter((item) => {
      ...
      // Date range filter (Column N - index 13 for Actual Date)
      let matchesDateRange = true
      if (startDate || endDate) {
        const itemDate = parseDateFromDDMMYYYY(item["col13"])
    ...
    .sort((a, b) => {
      // Sort by submission date (Column N - index 13)
      const dateStrA = a["col13"] || ""
      const dateStrB = b["col13"] || ""
      const dateA = parseDateFromDDMMYYYY(dateStrA)
      const dateB = parseDateFromDDMMYYYY(dateStrB)
      // Most recent first
      if (!dateA) return 1
  ...
  const confirmMarkDone = async () => {
    ...
    try {
      ...
      formData.append("sheetName", "Housekeeping")
      ...
      if (result.success) {
        ...
        setHistoryData((prev) =>
          prev.filter((item) => !selectedHistoryItems.some((selectedItem) => selectedItem._id === item._id)),
        )
  ...
  const fetchSheetData = async () => {
    try {
      ...
      const response = await fetch(
        `https://docs.google.com/spreadsheets/d/1ZeQWagWsRaTJXmQVU9aURP8BmH7QgHuckLxir-dHc0Q/gviz/tq?tqx=out:json&sheet=Housekeeping`,
      )
      ...
      data.table.rows.forEach((row, rowIndex) => {
        ...
        const isUserMatch = userRole === "admin" || assignedTo.toLowerCase() === username.toLowerCase()

        // If not a match and not admin, skip this row
        if (!isUserMatch && userRole !== "admin") return

        // Safely get values from columns M, N, Q, and R
        const columnMValue = getCellValue(row, 12) // Planned Date
        const columnNValue = getCellValue(row, 13) // Actual Date
        const columnQValue = getCellValue(row, 16) // Remarks
        const columnRValue = getCellValue(row, 17) // Uploaded Image

        // Skip rows marked as DONE in column R
        if (columnRValue && columnRValue.toString().trim() === "DONE") {
          return
        }

        // Convert column M value to string and format properly
        const rowDateStr = columnMValue ? String(columnMValue).trim() : ""
        ...
        // Check if column M is not null/empty and column N is null/empty
        const hasColumnM = !isEmpty(columnMValue)
        const isColumnNEmpty = isEmpty(columnNValue)

        // For pending tasks: Column M is not null and column N is null
        if (hasColumnM && isColumnNEmpty) {
        ...
        // For history: Both column M and N are not null
        else if (hasColumnM && !isColumnNEmpty) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submissionData = await Promise.all(
        selectedItems.map(async (id) => {
          ...
          return {
            ...
            additionalInfo: additionalData[id] || "", // Column P (Status)
            remarks: remarksData[id] || "", // Column Q (Remarks)
            imageData: imageData, // Column R (Uploaded Image)
            folderId: DRIVE_FOLDER_ID,
            // Add today's date for column N (Actual date)
            todayDate: todayFormatted, // Column N (Actual)
          }
        }),
      )
      ...
      if (result.success) {
        setAccountData((prev) =>
          prev.map((item) => (selectedItems.includes(item._id) ? { ...item, status: "completed", image: null } : item)),
        )

        setSuccessMessage(
          `Successfully processed ${selectedItems.length} account records! Columns N, P, Q and R updated.`,
        )
        setSelectedItems([])
        setAdditionalData({})
  ...
  return (
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-purple-200 shadow-md bg-white overflow-hidden">
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-purple-100 p-4">
            <h2 className="text-purple-700 font-medium">{showHistory ? "Completed Admin Records" : "Admin Records"}</h2>
                    ...
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <input
                          ...
                          checked={
                            filteredHistoryData.length > 0 && selectedHistoryItems.length === filteredHistoryData.length
                          }
                          onChange={(e) => {
                            if (e.target.checked) {
                      ...
                      {/* Render headers for columns B to N - EXCLUDE column O and R */}
                      {sheetHeaders.slice(1, 14).map((header) => (
                        <th
                          key={header.id}
                          className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider
      ${header.id === "col12" ? "bg-yellow-50" : ""} /* Planned Date */
      ${header.id === "col13" ? "bg-green-50" : ""} /* Actual Date */
    `}
                        >
                          {header.label}
                        </th>
                      ...
                      {/* Skip column O (index 14) and show P and Q */}
                      {sheetHeaders.slice(15, 17).map((header) => (
                        <th
                          key={header.id}
                          className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider
      ${header.id === "col15" ? "bg-blue-50" : ""} /* Status */
      ${header.id === "col16" ? "bg-purple-50" : ""} /* Remarks */
    `}
                        >
                          {header.label}
                        </th>
                    ...
                    {filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((history) => (
                        <tr key={history._id} className="hover:bg-gray-50">
                          ...
                          {/* Render data for columns B to N */}
                          {sheetHeaders.slice(1, 14).map((header) => (
                            <td
                              key={header.id}
    ...
    `}
                            >
                              <div className="text-sm text-gray-900">{history[header.id] || "—"}</div>
                            </td>
                          ))}
              ...
              <div className="p-4 border-b border-purple-100 bg-gray-50">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex items-center">
                        <label htmlFor="main-start-date" className="text-sm text-gray-700 mr-1">
                          From
                        </label>
                        <input
                          id="main-start-date"
                      ...
                      <div className="flex items-center">
                        <label htmlFor="main-end-date" className="text-sm text-gray-700 mr-1">
                          To
                        </label>
                        <input
                          id="main-end-date"
                  ...
                  {/* Clear Filters Button */}
                  {(mainStartDate || mainEndDate || searchTerm) && (
                    <button
                      onClick={resetFilters}
                      className="px-3 py-1 bg-red-100 text-red-700 rounded-md hover:bg-red-200 text-sm"
                    >
                    ...
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <input
                          ...
                          className="h-4 w-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500"
                          checked={
                            filteredAccountData.length > 0 && selectedItems.length === filteredAccountData.length
                          }
                          onChange={(e) => {
                            if (e.target.checked) {
                      ...
                      {/* Render headers for columns B to L (including Shift) */}
                      {sheetHeaders.slice(1, 12).map((header) => (
                        <th
                          key={header.id}
                          className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        >
                    ...
                    {filteredAccountData.length > 0 ? (
                      filteredAccountData.map((account) => (
                        ...
                        >
                          ...
                          {/* Render data for columns B to L (including Shift) */}
                          {sheetHeaders.slice(1, 12).map((header) => (
                            <td key={header.id} className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{account[header.id] || "—"}</div>
                            </td>
                          ))}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap bg-green-50">
                            {account.image ? (
                              ...
                              <label
                                className={`flex items-center cursor-pointer ${account["col10"]?.toUpperCase() === "YES" ? "text-red-600 font-medium" : "text-purple-600"} hover:text-purple-800`}
                              >
                      ...
                      <tr>
                        <td colSpan={sheetHeaders.length + 3} className="px-6 py-4 text-center text-gray-500">
                          {searchTerm || mainStartDate || mainEndDate
