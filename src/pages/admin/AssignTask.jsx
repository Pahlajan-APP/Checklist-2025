...
export default function AssignTask() {
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1ZeQWagWsRaTJXmQVU9aURP8BmH7QgHuckLxir-dHc0Q/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
        ...
        if (row.c && row.c[0] && row.c[0].v) {
          ...
          if (typeof dateValue === "string" && dateValue.startsWith("Date(")) {
            ...
            if (match) {
              ...
              dateValue = `${day.toString().padStart(2, "0")}/${(month + 1)
                .toString()
                .padStart(2, "0")}/${year}`;
            }
  ...
  const findNextWorkingDay = (date, workingDays, usedWorkingDays) => {
    ...
    if (
      workingDays.includes(formattedDate) &&
      !usedWorkingDays.includes(formattedDate)
    ) {
    ...
    while (attempts < maxAttempts) {
      ...
      if (
        workingDays.includes(nextDateFormatted) &&
        !usedWorkingDays.includes(nextDateFormatted)
      ) {
...
const generateTasks = async () => {
  ...
  if (formData.frequency === "one-time") {
    ...
    const [day, month, year] = taskDateStr.split('/').map(Number);
    ...
    while (currentIndex < futureDates.length) {
      ...
      switch (formData.frequency) {
        ...
        case "fortnightly":
          ...
          const [day2, month2, year2] = taskDateStr.split('/').map(Number);
          const currentDate2 = new Date(year2, month2 - 1, day2);
        ...
        case "monthly":
          ...
          const [day3, month3, year3] = taskDateStr.split('/').map(Number);
          const currentDate3 = new Date(year3, month3 - 1, day3);
        ...
        case "quarterly":
          ...
          const [day4, month4, year4] = taskDateStr.split('/').map(Number);
          const currentDate4 = new Date(year4, month4 - 1, day4);
        ...
        case "yearly":
          ...
          const [day5, month5, year5] = taskDateStr.split('/').map(Number);
          const currentDate5 = new Date(year5, month5 - 1, day5);
        ...
        case "end-of-last-week":
          ...
          const [day6, month6, year6] = taskDateStr.split('/').map(Number);
          const currentDate6 = new Date(year6, month6 - 1, day6);
...
const findClosestWorkingDayIndex = (workingDays, targetDateStr) => {
  ...
  for (let i = 0; i < workingDays.length; i++) {
    const [day, month, year] = workingDays[i].split('/').map(Number);
    const currentDate = new Date(year, month - 1, day);
...
const findEndOfWeekDate = (date, weekNumber, workingDays) => {
  ...
  for (const dateStr of daysInMonth) {
    ...
    const weekDay = date.getDay(); // 0 for Sunday, 1 for Monday, etc.
